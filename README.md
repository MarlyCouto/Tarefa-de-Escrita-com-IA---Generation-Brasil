...
1. Clone o repositório oficial do projeto:
   ```bash
   git clone [https://github.com/netguard-solutions/netguard-pro.git](https://github.com/netguard-solutions/netguard-pro.git)
   cd netguard-pro

   Conceda permissão e execute o script de instalação automatizado:
...
💡 Nota: Abra o seu navegador e acesse http://localhost:8080 para definir a sua senha de administrador master no primeiro acesso.
...
O administrador de TI acessa a aba Network Analytics do painel.

Filtra o tráfego das últimas 2 horas e identifica instantaneamente um volume anormal de requisições ICMP síncronas vindo de uma sub-rede externa.

Através da interface CLI do NetGuard Pro, o operador executa o comando isolado de contenção:
...
Core Engine: Escrito em Go para garantir máxima performance na filtragem de pacotes com baixo uso de CPU/Memória.
...
Valide os padrões de estilo do projeto executando os linters: npm run lint ou go fmt ./....

Envie as suas alterações via Pull Request (PR) descrevendo com detalhes o que foi modificado e associando à Issue técnica correspondente.

Para diretrizes completas sobre padrões e conduta, leia o nosso arquivo CONTRIBUTING.md.
...
Canal de Bugs: Abra uma nova Issue detalhando o problema encontrado, ambiente utilizado e os logs de erro anexados.
